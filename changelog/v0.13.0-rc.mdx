...
<Update label="v0.13.0-rc" description="Move work items between projects and Spaces, and approve every change CollabaseAI makes">
  ...
  * **Phone numbers ask for a country code:** The phone field on your profile, and the one administrators use in user management, now expects an international number such as `+41 44 123 45 67`. Spaces and dashes are fine and are tidied up on save. A number without a country code is refused, and the field says so instead of saving something that cannot be dialled. The field stays optional. See [Your Profile](/concepts/profile).

  ### 🎨 Layout & UX
  * **New icons across the modules:** Projects, Docs, Tests, Times, Registry, Automation and the Intranet have new tiles in the sidebar, and CollabaseAI has a new mark in the chat and on its button.
  * **A calmer CollabaseAI input box:** The chat input now sits on a filled panel instead of a bordered card, and the note about verifying answers moved above the field where it is read before typing rather than after.
  * **Personal settings no longer close on a stray click:** Clicking next to the personal settings window used to close it and discard anything you had typed but not saved. It now stays open. Close it with the **✕** or the Escape key as before.
