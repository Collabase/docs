...
| **Infomaniak** | Yes, with `Qwen/Qwen3-Embedding-8B`, which is asked for 768 dimensions |
...
Infomaniak offers three embedding models, and only one of them fits:

| Infomaniak model | Dimensions | Usable |
|---|---|---|
| `Qwen/Qwen3-Embedding-8B` | configurable up to 4096 | **Yes** — Collabase asks for 768 |
| `bge_multilingual_gemma2` | 3584, fixed | No |
| `mini_lm_l12_v2` | 384, fixed | No |

The two fixed-width models report a dimension mismatch on the status panel instead of indexing anything.
