...
| **Code runner** | Executes code blocks in automation workflows in an isolated sandbox |
| **AI engine** | Powers the Brain feature — local AI search, chat, and document analysis |
...
<Note>
  The AI engine is only started on servers with at least 4 GB of RAM. On smaller servers, all features except Brain work normally.
</Note>
...
|---|---|---|
| Standard (without AI) | 2 GB | 3 GB |
| With AI (Brain feature) | 4 GB | 6 GB |

The installer detects your available RAM and automatically starts the right configuration. You do not need to select this manually.
...
```bash
docker compose -f deployment/docker-compose.release.yml logs <service-name> --tail 50
```
