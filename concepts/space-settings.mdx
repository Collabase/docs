...
<Steps>
  ...
  <Step title="Assign a space role">
    Choose **Owner**, **Member**, or **Viewer**. See [Permissions](/concepts/permissions) for a breakdown of what each role can do.
  </Step>
...
The **AI** tab appears when Collabase Brain is enabled at the instance level (Admin → Settings → AI).

| Setting | Description |
|---|---|
| **AI Model Override** | Use a different model for this space instead of the instance default. Useful for giving a specific team access to a more capable model, or for cost control. |

Leave the override blank to use the global default model configured by the admin.
