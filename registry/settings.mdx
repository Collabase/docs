...
Visibility controls who can see this schema and the objects inside it. Because Registry is an instance-level module — not tied to any Space — visibility is defined at the instance level.

| Setting | Who has access |
|---|---|
| **Private** | Only you (the schema creator). No other user can see or access this schema or its objects. |
| **Public** | All authenticated users across the entire Collabase instance. |
...
<Note>
  Registry is not scoped to a Space. Space admins have no special access to Registry schemas — visibility is controlled exclusively by the **Private / Public** setting above.
</Note>
